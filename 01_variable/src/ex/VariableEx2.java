...
public class VariableEx2 {
	...
	public static void main(String[] args) {
		/* --- 컴퓨터 값 처리 원칙 ---
		 * - 같은 자료형 끼리만 연산이 가능하고 연산 결과도 같은자료형만 반환된다.
		 * --- 형 변환(Type Casting) ---
		 * - 값의 자료형을 변환하는 것
		 * 
		 * --- 자동 형변환(묵시적 형변환) ---
		 * 
		 * - 값의 범위가 다른 값들 끼리(다른 자료형들 끼리) 연산 시 "컴파일러(번역기)"가 자동으로 
	  ...
	  // 2) 컴파일러가 자동 형변환 수행 -> 값의 범위가 작은 int를 double 로 변환
		// 3) dobule + double
		
		// [int result1 = double] -> 자료형이 같지 않아 연산 불가 
		// -> 변수 선언 자료형 int처럼 명시한 자료형은 자동 형변환 대상 X -> 개발자가 직접 double로 변경해서 작성
		// int result1 = a1 + b1;
		...
		// 왜 결과가 1.5가 아니라 1??? 
		// - 값 처리 원칙에 의해서 int / int의 결과는 무조건 int가 되야되기 때문에
		//   컴파일러가 1.5(double)을 1(int)로 자동 형변환 -> 값의 범위가 관계없는 자동 형변환 예외 경우
