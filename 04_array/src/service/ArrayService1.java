...
public class ArrayService1 {
	...
	public void method1() {
		...
		// (참고)
		int num;
		// int 자료형을 저장하기위한 공간을 메모리에 할당하고
		// 해당 공간의 이름을 "num" 이라고 지정
		// -> 실제 정수 값을 저장 가능(실제 값을 저장할 수 있는 자료형은 기본 자료형 밖에 없음)
		...
		// -> 배열이 존재하는 위치(주소) 저장 가능(주소를 저장하는 변수를 " 참조형 변수" 라고 한다)
		
		/* 배열 할당 */
		arr = new int[4];
		// new : Heap 멤모리 영역에 객체/배열을 생성(할당)하는 연산자
	...
	public void method3() {
		String[] menus = {"순대국", "닭갈비", "햄버거","마라탕", "짜장면", "짬뽕",
											"포케", "샐러드"};
		// 0 ~ 배열 길이 -1사이의 난수 발생
	...
	public void method5() {
		...
		/* 최고, 최저점이 기록된 인덱스 찾기 */
		int maxIndex = 0; // 첫 번째 인덱스가 일단 최고/최저라고 기
		int minIndex = 0; // int i=0; i < scoreArr.length ; i++)	{
		...
		System.out.println("최소4값 : " + scoreArr[minIndex]);
	}
