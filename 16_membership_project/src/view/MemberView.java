...
public class MemberView {
	...
	public void addMember() throws IOException {
		...
		if(service.addMember(name, phone)) {
			System.out.println("*** 회원이 추가되었습니다 ***"); // true
		}else {
			System.out.println("### 중복되는 휴대폰 번호가 존재합니다 ###");  // false
		}
	...
	public void selectName() throws IOException {
		...
		List<Member> searchNameList = service.selectName(name);
	...
	public void updateAmount() throws IOException {
		...
		if(searchNameList.size() >= 2) {
			...
			for(int i=0; i<searchNameList.size(); i++) {
				System.out.printf("%d) %s (%s)\n", i+1, searchNameList.get(i).getName(), searchNameList.get(i).getPhone());
			}
			System.out.print("선택할 회원의 번호를 입력 : ");
			num = sc.nextInt()-1;
		}
		Member target = searchNameList.get(num);
		
		
		System.out.println("누적할 금액 입력 :");
		int money = sc.nextInt();
		System.out.println(service.updateAmount(target, money));
