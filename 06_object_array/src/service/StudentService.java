...
/* Service : 프로그램의 실질적인 기능(비지니스 로직)을 제공하는 클래스
 * 
 */
public class StudentService {
	...
	public StudentDTO selectName(String targetName) {
		...
		for(StudentDTO std : students) {
			...
			// boolean A.equals(B)
			// - A가 참조하는 객체의 필드와 B가 참조하는객체의 필드가 같은지 비교하여
	...
	/** 전달 받은 indext가 students 범위 내 인덱스가 맞는지
	...
	public String deleteStudent(int index) {
		...
		// 삭제될 학생의이름( 삭제 전 이름만 백업)
		String studentName = students[index].getName();
		
		// students배열에서 입력 받은 index 번째 요소를 null로 바꿈
		students[index] = null;
