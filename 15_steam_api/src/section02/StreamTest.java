...
public class StreamTest {
	public static void main(String[] args) {
		...
		/* java.util.Optinal<T> min(), max() : 최소/최대값 반환 */
		...
		/* 중간 연산
		 * - 기존 스트림을 이요해서 새로운 스트림을 만드는 연산
		 * 
		 */
		
		/* filter() : 기존 스트림에서 조건에 맞는 요소만을 이용해
		 * 새로운 스트림 생성 */
		...
		/* map()
		 * - 스트림의 모든 요소를 새로운 형태로 반환 후 새 스트림으로 반환ㄴ
		 * 
		...
		List<Integer> list = Arrays.asList(55, 33, 65, 78, 98);
		System.out.println();
		list.stream()
		.sorted() // 정렬
		.forEach(num -> System.out.print(num + " "));
