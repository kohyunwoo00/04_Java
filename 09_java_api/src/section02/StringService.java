...
public class StringService {
	 ...
	 * - String은 문자열을 저장하는 필드가 final로선언되어있음
	 * -> 한 번만 값 대입 가능
	 * 
	 * - 저장된 문자열 수정이 불가하기 때문에 
	 *   수정을 원할 경우 새로운 String 객체를 생성해서 참조
	 *   
	 *   ex) String s1 = "hello";   // 참조 주소 :0x00
	 *        s1 +="world";	         
	 */
	 ...
	 * ex)		String s1 = "hello";
	 * 	 -> heap 메모리 영역에 존재하는 String constant(상수) Pool String 객체 생성
	 * 			
	 * 2) new 연산자를 이용하여 생성
	 ...
	 * 
	 * - String 리터럴을 이용해서 [동일한 값]의 
	 *   String 객체를 생성하는 경우 새로운 객체가 생성되지 않고
	...
	public void test2() {
		...
		System.out.println("[필드 값 비교] s1.equals : " + s1.equals(s2)); // true
		System.out.println("[필드 값 비교] s1.equals : " + s1.equals(s3)); // true
		 ...
		 * 
		 * - 저장되는 문자열의 길이에 따라서 자동으로 크기가 증/감소)
		 * 
		 * - 문자열의 추가/수정/삭제가 발생해도 별도의 String 객체 생성이
		 * 	 없어서 매모레 효율이좋다
		 * 
		 * - StringBulder : Thread Safe 미지원(비동기, 빠름)
		 * - StringBuffer : Thread Safe 미지원(동기, 느림)
		 */
	...
	public void test3() {
		...
		/* delete(int start, int end) : start ~ end미만 사이 삭제 */
		sb.delete(4, 9);
		System.out.println("-----------------------------------------------");
		System.out.println("sb.toString() : " + sb.toString());
		System.out.println("sb 크기 : " + sb.capacity());
