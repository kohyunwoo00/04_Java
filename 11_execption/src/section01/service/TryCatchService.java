...
public class TryCatchService {
	 ...
	 * 발생한 예외를 처리하는 구문(try-catch, throws)
	 */
	...
	public void test2() {
		...
		try {
			...
			// catch는 여러 번 작성 가능
			// -> try에서 예외 발생 시 던져진 예외를 잡아서
			// 처리할 수 있는 catch문을 위에서 부터 순서대로 탐색
	...
	public void test3() {
		...
		try {
			...
			// ArithmeticException과 InputMisathException의
			// 공통 부모 타입을 매개 변수로 선언
	 ...
	 * 
	 * - catch문은 하위 타입부터 작성
	 * - 서로 상속 관계가 아닌 Exception에 대한 catch 순서는 크게 관계 없음
	 */
	public void test4() {
		...
		try {
			...
			System.out.println("0으로 나눌 수 없습니다");
		} catch(RuntimeException e) { // 부모 타입 예외 처리는 나중에 처리
	...
	public void test5() {
		...
		try {
			...
			System.out.println("나머지 모든 예외 잡아서처리");
		}
